...
def concat_first_last_digit(input_string):
    # Find the first occurrence of a digit
    first_digit_index = next((index for index, char in enumerate(input_string) if char.isdigit()), None)
    
    listWords = ['one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine']
    firstWord = lastWord = None
    first_word_index = 100000

    for word in listWords:
      temp = input_string.find(word)
      if temp < first_word_index and temp != -1:
          first_word_index = temp
          firstWord = word
    

    # Find the last occurrence of a digit
    last_digit_index = next((index for index, char in enumerate(reversed(input_string)) if char.isdigit()), None)
    
    last_word_index = 0
    
    for word in listWords:
      temp2 = input_string.rfind(word)
      if temp2 > last_word_index and temp2 != -1 :
          last_word_index = temp2
          lastWord = word
          
    actualLastDigitIndex = len(input_string) - 1 - last_digit_index
    
    firstDigit = input_string[first_digit_index]  
    lastDigit = input_string[actualLastDigitIndex]
    if actualLastDigitIndex < last_word_index:
        lastDigit = wordToDigit(listWords, lastWord)
    
    if first_digit_index > first_word_index:
        firstDigit = wordToDigit(listWords, firstWord)
        
    result = str(firstDigit) + str(lastDigit)
    
    return result
    
def wordToDigit(list, word):
    for index, listWord in enumerate(list):
        if listWord == word: 
            return index +1
        
    
def concat_first_last_digit_or_spelled_out(input_string):
    # Regular expression to find words in the input string
    word_pattern = re.compile(r'\b(?:\d+|one|two|three|four|five|six|seven|eight|nine)\b')

    # Find all words in the input string
    words = re.findall(word_pattern, input_string.lower())

    # Find the first occurrence of a digit or spelled-out digit
    first_digit_index = next(
        (index for index, word in enumerate(words) if word.isdigit() or word in ['one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine']),
        None
    )

    # Find the last occurrence of a digit or spelled-out digit
    last_digit_index = None
    for index, word in enumerate(reversed(words)):
        if word.isdigit() or word in ['one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine']:
            last_digit_index = len(words) - 1 - index
            break

    # If both first and last digits are found
    if first_digit_index is not None and last_digit_index is not None:
        # Concatenate the first and last digits
        result = words[first_digit_index] + words[last_digit_index]
        return result
    else:
        # Return a message if no digits are found
